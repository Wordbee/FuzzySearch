  ...
  /** @lends {FuzzySearch.prototype} */ {
    ...
    _prepItem: function(source_item, keys) {
      ...
      for (var field_index = -1; ++field_index < nb_fields; ) {
        var field = item_fields[field_index];
        for (
          var node_index = -1, nb_nodes = field.length;
          ++node_index < nb_nodes;
          ...
          if (norm.length > 2 * this.options.token_field_min_length)
            nodes = FuzzySearch.filterSize(
              nodes,
              this.options.token_field_min_length,
              this.options.token_field_max_length
            );
          if (this.options.score_acronym)
            nodes.push(norm.replace(this.acro_re, "$1"));
    ...
    add: function(source_item, should_update_source) {
      ...
      if (should_update_source === undefined) should_update_source = true;

      var item_id =
        typeof this.options.identify_item === "function"
      ...
      // Insert in source;
      if (should_update_source) this.source[idx] = source_item;
    ...
    findWordByIndex: function(string) {
      return this.source.indexOf(string);
    },
  ...
  if (level === nb_level) {
    if (isArray)
      for (i = -1, olen = obj.length; ++i < olen; )
        list.push(toStringOrDefault(obj[i], ""));
    else if (isObject) {
      for (key in obj) {
